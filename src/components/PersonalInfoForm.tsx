...
const PersonalInfoForm: React.FC<PersonalInfoFormProps> = ({ 
  ...
  const countries = ['USA', 'ECUADOR', 'COLOMBIA', 'BRASIL'];
  
  const cities = {
    USA: ['FLORIDA', 'WASHINGTON', 'NEBRASKA', 'COLORADO'],
    ECUADOR: ['QUITO'],
    COLOMBIA: ['BOGOTÁ', 'MEDELLIN'],
    BRASIL: ['BETIM', 'SAO PAULO CAPITAL', 'CAMBORIU', 'CURITIBA', 'MARINGA', 'FARRUPILHA']
  };
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Render first 6 fields */}
        {formFields.slice(0, 6).map(renderField)}
        ...
        <div className="mb-3">
          ...
          <select
            ...
            required
            disabled={readOnly || !personalInfo.pais}
          >
            <option value="">{t('selectCity')}</option>
            {personalInfo.pais && cities[personalInfo.pais as keyof typeof cities]?.map((city) => (
